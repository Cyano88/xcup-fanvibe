...
export const DEFAULT_SEASON_TIMING: SeasonTiming = {
  preseasonSeconds: SEASON_PRESTART_SECONDS,
  matchMs: GROUP_STAGE_MATCH_MS,
  matchdayGapMs: SEASON_MATCHDAY_GAP_MS,
  interseasonSeconds: SEASON_INTERMISSION_SECONDS * 2,
  waveGapMs: SEASON_WAVE_GAP_MS,
};

export const TEST_SEASON_TIMING: SeasonTiming = {
  preseasonSeconds: 60,
  matchMs: 60 * 1000,
  matchdayGapMs: 60 * 1000,
  interseasonSeconds: 60,
  waveGapMs: 0,
};

let activeSeasonTiming: SeasonTiming = DEFAULT_SEASON_TIMING;

export function setSeasonTiming(timing: SeasonTiming): void {
  activeSeasonTiming = timing;
}

export function getSeasonTiming(): SeasonTiming {
  return activeSeasonTiming;
}
...
export function seasonFixtureKickoffDelayMs(fixtures: Fixture[], fixtureId: string): number {
  ...
  if (index < 0) return 0;
  const timing = getSeasonTiming();
  const matchdayDelay = fixture ? (fixture.matchday - 1) * (timing.matchMs + timing.matchdayGapMs) : 0;
  return matchdayDelay + Math.floor(index / SEASON_WAVE_SIZE) * getSeasonTiming().waveGapMs;
}
...
export function seasonFixtureStartAtMs(
  ...
  const waveDelay = Math.floor(Math.max(0, sameDayFixtures.findIndex(f => f.id === fixture.id)) / SEASON_WAVE_SIZE) * getSeasonTiming().waveGapMs;
  ...
  const latestPreviousFinish = Math.max(
    ...previousMatchdays.map(f => matchStates[f.id]?.finishedAt ?? seasonStartedAt + seasonFixtureKickoffDelayMs(fixtures, f.id) + getSeasonTiming().matchMs),
  );
  return latestPreviousFinish + getSeasonTiming().matchdayGapMs + waveDelay;
}
