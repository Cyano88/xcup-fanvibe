      ...
      <div className="relative z-10">
        ...
        <div className="mt-4 border-t border-white/10 pt-4">
          <div className="flex flex-wrap items-center justify-between gap-3">
            <div className="inline-flex rounded-md bg-black/35 p-0.5">
              <button
                type="button"
                onClick={() => setActiveBoard('holders')}
                className={`rounded px-3 py-1.5 text-xs font-bold transition-colors ${activeBoard === 'holders' ? 'bg-white text-zinc-950' : 'text-zinc-300 hover:text-white'}`}
              >
                Top FVB holders
              </button>
              <button
                ...
                className={`rounded px-3 py-1.5 text-xs font-bold transition-colors ${activeBoard === 'countries' ? 'bg-white text-zinc-950' : 'text-zinc-300 hover:text-white'}`}
              >
                Country backing
              </button>
            </div>
          ...
          <div className="mt-3 overflow-hidden rounded-lg border border-white/10 bg-black/25 backdrop-blur-[2px]">
            {activeBoard === 'holders' ? (
              visibleHolderRows.length === 0 ? (
                <div className="px-3 py-8 text-center text-sm text-zinc-300">
                ...
                return (
                  <a
                    ...
                    className="grid min-w-0 grid-cols-[auto_1fr_auto] items-center gap-3 border-b border-white/10 px-3 py-3 text-sm last:border-b-0 transition-colors hover:bg-white/[0.04]"
                  >
                    <div className="grid h-7 w-7 place-items-center rounded bg-white text-xs font-black text-zinc-950">{entry.rank}</div>
                    <div className="min-w-0">
                    ...
                    <div className="text-right">
                      <div className="text-xs font-bold tabular-nums text-white">{compactTokenBalance(entry.eligibleWei)} FVB</div>
              ...
              return (
                <div key={entry.code} className="grid min-w-0 grid-cols-[auto_auto_1fr_auto] items-center gap-3 border-b border-white/10 px-3 py-3 last:border-b-0">
          ...
          {hasMoreRows && (
            <button
              ...
              className="mt-3 w-full rounded-md border border-white/10 px-3 py-2 text-xs font-bold text-zinc-300 transition-colors hover:border-blue-300/50 hover:text-blue-100"
            >
              Show 20 more
            </button>
          )}
